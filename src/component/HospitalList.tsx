...
export default function HospitalList({ hospitals }: HospitalListProps) {
  ...
  const handleBookHospital = async (hospital: Hospital) => {
    ...
    try {
      const bookingId = `${hospital.id}_${Date.now()}`; // Unique ID for the booking
      const bookingRef = doc(db, 'users', currentUser.uid, 'bookings', bookingId);
      const bookingData = {
        hospitalId: hospital.id,
        hospitalName: hospital.name,
        date: new Date().toISOString(), // In a real app, let the user select a date
        status: 'pending',
      ...
      if (error.code === 'unavailable') {
        toast.error("You are offline. The booking will be created when you're back online.");
      } else {
  ...
  return (
    <div>
      <button
        onClick={exportToCSV}
        className="mb-6 bg-green-500 hover:bg-green-600 text-white py-2 px-4 rounded-md transition-colors flex items-center justify-center"
      >
        <svg
